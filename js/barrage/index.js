let data = [
  ...
  class CanvasBarrage {
    constructor(canvas, video, opts = {}) {
      ...
      //合并对象 将opts合并到defOpts 再挂载到this上
    ...
    render() {
      ...
      if (this.isPaused === false) {
        requestAnimationFrame(this.render.bind(this))  //每隔16.7ms重新调用一下render
      }
    }
  ...
  class Barrage {
    ...
    init() {
      ...
      // 计算每条弹幕的宽度
      let p = document.createElement('p')  //创建一个p便签
      p.style.fontSize = this.fontSize + 'px'
      p.innerHTML = this.value  //将vaule 装入 p 标签
      document.body.appendChild(p)
      this.width = p.clientWidth   //将此时p容器的宽度赋予width进行初始化
      document.body.removeChild(p)  // 移除p标签
    ...
    // 渲染每一条弹幕
    renderOnce() {
      this.context.ctx.font = `${this.fontSize}px Arial`  //云括号 模板拼接
