<template>
  <div class="container">
    ...
    <div class="future" v-if="futureMapData.length > 0">
      <!-- v-if指令 若数组的长度大于0时才 future的dom结构才进行展示 -->
      <div class="group">
        明天:
        <span class="tm"
...
export default {
  ...
  methods: {
      ...
      AMap.plugin("AMap.CitySearch", function () {
        ...
        citySearch.getLocalCity(function (status, result) {
          if (status === "complete" && result.info === "OK") {
            ...
            // console.log(result);
            _this.getCurrentCityData(result.city);  //将输出结果的city作为实参传给getCurrentCityData的cityname
          }
        });
      });
    },
    getCurrentCityData(cityName) {
      ...
      AMap.plugin("AMap.Weather", function () {
        ...
        weather.getLive(cityName, function (err, data) {
          ...
          _this.mapData = data; // 将返回的data对象值传给mapData
        });
        //执行实时天气信息查询
    ...
    initEchart() {
      let dom = this.$refs.echartContainer;  //$refs拿到ref为echartContainer的dom结构
